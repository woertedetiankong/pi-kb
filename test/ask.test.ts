...
test("ask retrieves passages, answers from them and keeps only the cited sources, renumbered", async () => {
	...
	const r = await ask(new Library(kb), ctx, "CTRL_REG 复位后是多少？", new AbortController().signal);
...
test("ask skips the answering call when nothing matches, and needs a model", async () => {
	const calls: { system: string; text: string }[] = [];
	const r = await ask(new Library(kb), fakeModel('{"queries":["zebra migration"]}', () => "should not be called", calls), "Where do zebras migrate?", new AbortController().signal);
	assert.equal(calls.length, 1);
	assert.deepEqual([r.answer, r.sources], ["", []]);
	await assert.rejects(ask(new Library(kb), undefined, "x", new AbortController().signal), (e: unknown) => e instanceof AskError && e.problem === "no_model");
	const failing = {
		model: { id: "m", maxTokens: 100 },
		modelRegistry: { complete: async () => ({ stopReason: "error", errorMessage: "quota", content: [] }) },
	} as unknown as ModelContext;
	await assert.rejects(ask(new Library(kb), failing, "x", new AbortController().signal), (e: unknown) => e instanceof AskError && e.problem === "model_failed" && e.message === "quota");
});
...
test("the page can pick another model; unknown or unconfigured models are refused", async () => {
	...
	const r = await ask(new Library(kb), ctx, "CTRL_REG", new AbortController().signal, "cheap/small/v2");
	...
	assert.equal((await ask(new Library(kb), ctx, "CTRL_REG", new AbortController().signal)).model, "test/fake", "no pick follows pi");
	const refused = (key: string, problem: string) =>
		assert.rejects(ask(new Library(kb), ctx, "x", new AbortController().signal, key), (e: unknown) => e instanceof AskError && e.problem === problem);
