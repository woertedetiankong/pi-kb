...
pi install git:github.com/woertedetiankong/pi-kb@v0.5.1
...
```bash
node scripts/model-check/run.ts                      # default openai-codex/gpt-6-luna, 15 scenarios, 2 runs each
...
It calls the model for real and is billed (15 × 2 runs take a few minutes). Every run's result and full answer go to `report.md` in the output folder.
