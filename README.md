...
常用命令（在 pi 里输入 `/kb ` 时，补全列表只显示这些；`/kb help` 列出全部）：

| 命令 | 作用 |
|---|---|
| `/kb add <文件或文件夹…> [--project\|--global]` | 在后台导入资料，命令立即返回，可以继续对话；支持拖拽路径。在有项目知识库的项目里可指定放进项目还是全局 |
| `/kb add <笔记.md…> --note` | 作为笔记导入（不加 `--note` 时 Markdown 当作资料导入，汇总里会提示；网页上拖入的 Markdown 则默认当笔记） |
| `/kb search <关键词>` | 自己搜一下 |
| `/kb web` | 在浏览器中打开知识库页面（`/kb web url` 显示完整地址，`/kb web stop` 关闭） |
| `/kb note [重点]` | 让 agent 回顾本次对话，把值得保留的经验写成笔记 |
| `/kb list [关键词]` | 列出资料和笔记（最多显示 50 项；加关键词按标题筛选） |
| `/kb remove <标题或 id>` | 删除（笔记会连文件一起删） |
| `/kb cancel` | 取消正在进行的导入（已导入完的文件保留） |
| `/kb` 或 `/kb status` | 查看开关状态和收录数量 |
| `/kb on` / `/kb off` | 开启 / 关闭（持久保存）。关闭后工具和提示词都会移除，不占上下文 |
| `/kb help` | 列出全部命令 |

更多命令（项目知识库、检索设置、维护；输入开头几个字母时也会出现在补全里）：

| 命令 | 作用 |
|---|---|
| `/kb init` | 在项目里创建项目知识库（`.pi/kb`，通过 git 与团队共享），见下文 |
| `/kb move <标题或 id> [project\|global]` | 把资料或笔记移到项目知识库 / 全局知识库（不写目标就移到另一边） |
| `/kb use [名字… \| all \| none]` | 这个项目使用全局库里的哪些资料集（见下文「资料集」）；不带参数时列出全部资料集 |
| `/kb group <标题或 id> <资料集>` | 把资料或笔记放进资料集（`-` 表示不分组）；`/kb group --rename <旧名> <新名>`、`/kb group --delete <名字>` 改名或删除资料集 |
| `/kb semantic [status\|api\|local\|off\|remove]` | 语义检索：查看状态、用在线接口、用本机模型、关闭、删除本机模型（释放约 1.1GB，资料和笔记保留） |
| `/kb reread <标题或 id>` | 用当前的 OCR 设置、按原文件重新识别一份 PDF、图片或 Office 资料（改了识别语言或 OCR 服务之后）。保留 ID、标题和导入时间，在后台进行 |
| `/kb lint` | 检查笔记：可能重复的、失效的 `[[链接]]`、链接到全局笔记的项目笔记（队友打不开）、没有标签的 |
| `/kb eval [init\|draft\|run]` | 评测检索效果：建问题文件、让 agent 起草问题、运行评测 |
| `/kb open` | 打开知识库文件夹 |
| `/kb lang zh\|en\|auto` | 切换界面语言 |
| `/kb sync` | 立即重新读取文件夹。一般用不到：手动改的笔记、其他电脑或同事同步来的内容会自动发现 |
...
| 隐私 | **资料和笔记的文字会发送给服务商**（开启时会提示确认） | 全部留在本机 |
| 费用 | 按服务商计费 | 无 |
...
2. 按格式写问题，每行一个：
   ...
   竖线右边是答案所在的资料（文件名或笔记标题的一部分，可加页码；多个可接受答案用 `;` 分隔），`-` 表示知识库里没有答案。问题最好用客户真实会问的说法，不要照抄原文。
...
- **放错了可以移**：`/kb move <标题或 id>`（移到另一边），或在网页上点“移到项目库 / 移到全局库”。
- **进 git 的**：笔记、转换后的文本（只有原 PDF 的 2%–7%）、资料描述。**不进 git 的**：原文件和本机的修改日志（见自动生成的 `.pi/kb/.gitignore`；想连原文件一起共享，删掉里面的 `raw/` 一行即可）。
...
全局库可以按主题分成几个**资料集**，比如「ESP32」「STM32」「公司规范」，每个项目选择自己用哪几个。这样做 ESP32 项目时，agent 不会被 STM32 的资料干扰，也就不容易引用错型号。

```
一个项目里 agent 能看到的 = 项目库（如果有）
                        + 全局库里没有归入任何资料集的内容    ← 个人偏好、通用经验：处处可见
                        + 全局库里属于本项目所选资料集的内容
```

- **默认全部可见**：没有资料集，或者项目没做过选择时，和以前完全一样。
- **归组**：`/kb add ~/资料` 导入文件夹时，会按子文件夹建议分组（`~/资料/ESP32/…` → 「ESP32」），先问你再分；直接放在所选文件夹下的文件不分组。也可以 `/kb add <路径> --to ESP32`，或者用 `/kb group <标题> ESP32` 给已有的资料归组。网页上在资料页的「资料集」一行点「修改」，导入时在左侧「放进资料集」里选。
- **一份资料可以属于多个资料集**；资料集的名字不区分大小写（`esp32` 就是「ESP32」）。
- **项目选择**：`/kb use ESP32 公司规范`，或者在网页侧栏「本项目使用」旁点「修改」。`/kb use all` 恢复全部，`/kb use none` 只用未分组的内容。选择记在本机的 `config.json` 里（按项目根目录），换一台电脑要再选一次。
- **agent**：只在本项目可见的范围里检索；你说「在 STM32 资料里查一下」时，它会用 `kb_search` 的 `shelf` 参数只查那个资料集。记经验时，和本项目某个资料集主题相关的，它会放进那个资料集（保存前的确认框里可以改）；个人偏好和通用经验不分组，所有项目都能看到。
- **存在哪**：资料的资料集记在 `docs/<id>.json` 里，笔记的记在 front matter 的 `shelves: [ESP32]` 里，所以会跟着同步盘到另一台电脑。资料集只用于全局库：项目库本身就只属于这个项目，资料移进项目库时会去掉资料集。

## 网页

`/kb web` 打开本地网页（只监听 `127.0.0.1`，需要访问令牌）：

- 把文件拖到页面上导入：Markdown 文件作为笔记，其他文件作为资料。想把 Markdown 当资料（比如软件的说明文档），点提示条上的「改为资料导入」；错过了也没关系，笔记页面上随时可以点「改为资料」，Markdown 资料页面上也有「改为笔记」。侧栏的「资料」「笔记」悬停时会说明两者的区别和 `/kb add` 共用同一个后台队列，页面上显示进度；终端里发起的导入，页面上也能看到进度
- 搜索结果带页码，点开直接定位到那一页；PDF 可以「看原页」在浏览器里打开到对应页
- 「✨ 问 AI」：根据知识库回答搜索框里的问题。默认用 pi 当前的模型，也可以在按钮旁边另选一个（记在浏览器里，便宜快速的模型通常就够用）。先让模型拆出几组检索词（含中英文和手册里的说法），再只根据检出的段落回答，每句标出处 [1]，点出处直接跳到原文那一页；资料里没有的会直说，不硬答。每问一次调用两次模型（约 1–2 千 token）。搜索结果少于 3 条时，结果下面会出现「用 AI 找找看」，直接对同一个问题问 AI
- 浏览转换后的原文（表格、标题按 Markdown 显示）、新建和编辑笔记、删除、开关知识库
- PDF、图片和 Office 资料的页面上有「重新识别」：改了 OCR 语言或服务后，用原文件重新转换，保留 ID、标题和导入时间，以前的引用仍然对得上。和导入走同一个队列；原文件不在本机时（项目库默认不提交原文件）不能重新识别
- 笔记里的 `[[链接]]` 可以点开（找不到的笔记显示为删除线）；点标签或侧栏的「标签」只看带这个标签的笔记
- 没开语义检索时，搜索结果很少会提示原因：只按字面匹配，中文搜不到英文资料
- 中文 / English 切换；链接加 `?lang=en` 或 `?lang=zh` 可指定语言，`?q=<关键词>` 直接搜索，`?doc=<id>&page=<n>` 直接打开某份资料的某一页
...
- `kb_search`：关键词检索，返回 `[手册.pdf p.12]` 形式的引用和资料 id
- `kb_read`：按 id 和页码读取原文（例如 `pages: "12-14"`）。加 `view: true` 时还会从原始 PDF、图片或 Office 文件渲染出页面图片（每次最多 4 页），让模型看到文字里丢失的框图、原理图、引脚图和表格排版。仅限支持图片输入的模型，且需要原件在本机（项目知识库默认不把原件提交到 git）。某页文字来自 OCR（扫描件、照片，或从图里识别出的文字）时，`kb_read` 会在开头注明，并建议先看页面再引用具体数值
...
- `kb_note`：把经验写成笔记。解决了不明显的问题（调试找到的根因、坑、变通办法）或了解到你的环境偏好时，agent 会主动调用；每条笔记都会先给你预览，选择 **Save / Edit, then save / Don't save**。同名笔记不会重复创建，而是追加（`append`）或改写（`replace`）已有笔记
...
system prompt 里会注入一个很小的目录（数量、笔记标题、最近导入的资料），让 agent 知道库里有什么。
...
raw/<id>/<原文件>        原件副本
converted/<id>.md        转换后的 Markdown，带 <!-- kb:page N --> 页码标记（文字来自 OCR 的页还有 <!-- kb:ocr 字数/总字数 -->）
docs/<id>.json           每份资料的描述（标题、来源、页数），索引据此重建
wiki/**/*.md             笔记，可直接用 Obsidian 或编辑器维护
...
**换存放位置**：资料和笔记可以放在任何文件夹。在网页「设置 → 存放位置」里填一个文件夹，可以选择把现有资料复制过去，其他 pi 窗口会自动跟着切换。选 iCloud、坚果云、Dropbox 里的文件夹，并让几台电脑都指向它，就能共用同一个知识库：一台电脑新加的资料和笔记，另一台几秒内就能搜到（pi 在下次检索、打开网页时发现文件变了，会自动建好索引）。
...
- Tesseract 识别中文扫描件质量一般：同一行的词序可能被打乱。扫描件多时建议配置 PaddleOCR 服务：在网页「设置」的 OCR 服务地址里填写，或在 `config.json` 里设置 `"ocrServerUrl"`（LiteParse 的 OCR HTTP 接口）。识别语言也可以在那里改，改完对之后导入的文件立即生效，不用重启 pi；已导入的资料在网页上点「重新识别」即可。如果用的是支持图片输入的模型，这一点影响小很多：OCR 只要能让搜索找到那一页就够了，agent 会先看页面再引用具体数值。
...
- 退出 pi、`/reload`、`/new`、`/resume` 会暂停正在进行的导入：已导入完的文件保留，没导完的记在本机的 `~/.pi/kb/pending-imports/`，下次 pi 启动时自动接着导入（网页上传的文件也会先复制一份）。当时正在转换的那个文件要从头开始。要导入到某个项目库的文件，等你在那个项目里启动 pi 时才继续。`pi -p` 这类非交互运行不会接手。
...
```bash
node scripts/model-check/run.ts                      # 默认 openai-codex/gpt-6-luna，17 个场景各跑 2 次
...
每次运行都复制一份全新的虚构资料库（XR-100 芯片手册、XR-100 评估板外形图、Orbit 部署手册、YF-20 打印机 FAQ、一条 SPI 踩坑笔记，外加几篇无关文档），在空项目目录里执行 `pi -p --mode json`，只加载本扩展，不会动你的 `~/.pi/kb`。场景在 `scripts/model-check/scenarios.ts`，检查：
...
- 答案只在图里时，是否用 `kb_read` 的 `view: true` 看页面：评估板外形图（`xr100-outline.pdf`，由同目录的 `xr100-outline.ts` 生成）的尺寸是用线条画的，OCR 只能认出一部分，竖排的尺寸（38、30）只能从页面图片上读到

会真实调用模型、产生费用（17 × 2 次约几分钟）。逐次结果和回答原文写在输出目录的 `report.md`。
...
加入看页面功能后（v0.5.4，2026-09-26，gpt-6-sol，外形图场景和另外三个 XR-100 场景各 3 次）：12/12。外形图场景每次都先搜索，再查看第 1 页，四个尺寸全部读对；资料库里多了这张图，也没有影响其他 XR-100 问题的回答。在 M5Stack StickS3 的真实资料上，文字里找不到答案时它也会自己去看尺寸图，文字里有答案时就直接用文字。
