...
- **导入和记经验默认放进项目库**：`/kb add --global` 放进全局库，网页左下角可以切换“新资料放到：项目 / 全局”。agent 记经验时自己判断：只和本项目有关的（编译烧录步骤、接线、团队约定）放项目库，通用的（芯片、工具知识）和个人偏好放全局库，保存前的确认框里可以一键改存到另一边。
...
- 同事拉取代码后启动 pi，会自动建索引，能搜索、问 AI、看带页码的文本；没有原文件时“查看原文件”不可用。之后别人推送的新内容，重启 pi 或 `/kb sync` 后可见。
