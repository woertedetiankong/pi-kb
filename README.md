...
示例结果（pi 自带文档 + 中文资料，502 个片段；29 个有答案的问题，其中 6 个是 `CTRL_REG`、`setActiveTools` 这类精确词，8 个没有答案的问题；本机 Qwen3）：

| 模式 | 第一条命中 | 前三命中 | 前八命中 | MRR | 无答案时返回空 |
|---|---|---|---|---|---|
| 关键词 | 34% | 34% | 48% | 0.37 | 88% |
| 关键词+语义 | 72% | 90% | 97% | 0.81 | 88% |
| 仅语义 | 69% | 86% | 97% | 0.79 | 100% |
...
- 查询里的 how / the 这类英文虚词和"怎么、如何、什么"这类疑问词会被忽略；有多个词时，至少命中一半才算匹配。
