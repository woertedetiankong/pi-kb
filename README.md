...
- 把文件拖到页面上导入：左半边作为资料，右半边（Markdown）作为笔记。和 `/kb add` 共用同一个后台队列，页面上显示进度；终端里发起的导入，页面上也能看到进度
...
- Tesseract 识别中文扫描件质量一般：同一行的词序可能被打乱。扫描件多时建议配置 PaddleOCR 服务：在网页「设置」的 OCR 服务地址里填写，或在 `config.json` 里设置 `"ocrServerUrl"`（LiteParse 的 OCR HTTP 接口）。识别语言也可以在那里改，改完对之后导入的文件立即生效，不用重启 pi。如果用的是支持图片输入的模型，这一点影响小很多：OCR 只要能让搜索找到那一页就够了，agent 会先看页面再引用具体数值。
