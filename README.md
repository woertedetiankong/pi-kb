...
- `kb_read`：按 id 和页码读取原文（例如 `pages: "12-14"`）。加 `view: true` 时还会从原始 PDF、图片或 Office 文件渲染出页面图片（每次最多 4 页），让模型看到文字里丢失的框图、原理图、引脚图和表格排版。仅限支持图片输入的模型，且需要原件在本机（项目知识库默认不把原件提交到 git）
