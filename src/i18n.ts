...
const en = {
	...
	pickTitle: (action: "remove" | "move" | "reread", shown: number, total: number) =>
		`${{ remove: "Remove which one?", move: "Move which one?", reread: "Read which document again?" }[action]}${total > shown ? ` (showing ${shown} of ${total}; add words from the title to narrow it down)` : ""}`,
...
const zh: Messages = {
	...
	pickTitle: (action, shown, total) =>
		`${{ remove: "要删除哪一项？", move: "要移动哪一项？", reread: "要重新识别哪一份资料？" }[action]}${total > shown ? `（显示 ${shown}/${total} 项，加上标题里的词可以缩小范围）` : ""}`,
