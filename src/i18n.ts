...
const en = {
	...
	status: (on: boolean, docs: number, pages: number, notes: number, root: string) =>
		`Knowledge base ${on ? "on" : "off"} · ${plural(docs, "doc")} (${plural(pages, "page")}) · ${plural(notes, "wiki note")} · ${root}`,
	usageAdd: "Usage: /kb add <file or folder…> [--note] [--project | --global]",
	...
	pickTitle: (action: "remove" | "move", shown: number, total: number) =>
		`${action === "remove" ? "Remove which one?" : "Move which one?"}${total > shown ? ` (showing ${shown} of ${total}; add words from the title to narrow it down)` : ""}`,
	...
	lintRunning: "📚 Checking wiki notes…",
	lintTitle: (n: number) => `🩺 Wiki check · ${plural(n, "note")}`,
	...
	subcommands: {
		...
		list: "List documents and wiki notes: /kb list [words]",
		search: "Search the knowledge base: /kb search <query>",
		note: "Save lessons from this conversation as wiki notes: /kb note [focus]",
		lint: "Check the wiki notes: likely duplicates, broken [[links]], notes without tags",
...
const zh: Messages = {
	...
	replacedOld: "已替换旧版本",
	versionAsk: (files) =>
		`${files.length === 1 ? "这个文件是" : `这 ${files.length} 个文件是`}知识库里已有文档的新版本：\n${files.map((f) => `  ${f}`).join("\n")}`,
	...
	usageAdd: "用法：/kb add <文件或文件夹…> [--note] [--project | --global]",
	projectStatus: (name, docs, notes, dir) => `项目知识库「${name}」（通过 git 与团队共享）：${docs} 份文档 · ${notes} 条笔记 · ${dir}`,
	...
	noteEditor: "编辑笔记",
	noteSimilar: "已有相似的笔记：",
	lintRunning: "📚 正在检查 wiki 笔记…",
	lintTitle: (n) => `🩺 Wiki 检查 · ${n} 条笔记`,
	...
	localRemoveBody: (size, inUse) =>
		`将删除模型运行时和模型文件（${size}）。你的文档、笔记和检索索引都会保留。${inUse ? "语义检索正在使用这个模型，会先关闭（只用关键词检索）。" : ""}以后执行 /kb semantic local 可以重新下载。`,
	...
	apiPrivacyTitle: "把文档内容发送给在线服务？",
	apiPrivacy: (host) => `为了建立语义索引，所有文档和笔记的文字都会发送给 ${host}；每次搜索也会发送你的查询。`,
	...
	evalDraft: (path) =>
		`请为我的知识库起草大约 15 个评测问题，追加到 ${path}。用 kb_search 和 kb_read 在不同的文档和笔记里找真实的段落。每个问题都写成用户自然会问的样子：用你自己的话，不要照抄原文里的词句，其中一部分用另一种语言（中文或英文）提问。每个问题后面写 " | <文档标题> p.<页码>"（笔记和没有页码的文件不写 p.）。另外加 3 个知识库回答不了的问题，结尾写 " | -"。保留文件开头的注释行。完成后告诉我加了多少个。`,
	...
	subcommands: {
		...
		cancel: "取消正在进行的导入",
		list: "列出文档和 wiki 笔记：/kb list [关键词]",
		search: "搜索知识库：/kb search <关键词>",
		note: "把本次对话的经验存为 wiki 笔记：/kb note [重点]",
		lint: "检查 wiki 笔记：可能重复的、失效的 [[链接]]、没有标签的",
		remove: "删除文档或笔记：/kb remove <标题或 id>",
